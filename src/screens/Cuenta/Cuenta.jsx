import React from "react";
import "./style.css";

export const Cuenta = () => {
  return (
    <div className="cuenta">
      <div className="div">
        <div className="overlap">
          <img className="rectangle" alt="Rectangle" src="/img/rectangle-2.png" />
          <div className="text-wrapper">Fecha de vencimiento</div>
          <div className="text-wrapper-2">05 de Abril 2024</div>
          <div className="text-wrapper-3">Detracción pendiente</div>
          <div className="text-wrapper-4">S/. 1127,00</div>
          <div className="text-wrapper-5">Total deuda</div>
          <div className="text-wrapper-6">S/. 53127,00</div>
          <div className="grupo">
            <div className="text-wrapper-7">Cuenta</div>
            <div className="text-wrapper-8">Estado de cuenta</div>
            <img className="chevron-right" alt="Chevron right" src="/img/chevron-right.png" />
          </div>
          <div className="overlap-wrapper">
            <div className="overlap-group">
              <div className="rectngulo" />
              <div className="componente">
                <div className="sun-wrapper">
                  <div className="sun">
                    <div className="overlap-group-2">
                      <div className="elipse" />
                      <img className="trazado" alt="Trazado" src="/img/trazado-2504.png" />
                      <img className="img" alt="Trazado" src="/img/trazado-2505.png" />
                      <img className="trazado-2" alt="Trazado" src="/img/trazado-2508.png" />
                      <img className="trazado-3" alt="Trazado" src="/img/trazado-2509.png" />
                    </div>
                    <img className="trazado-4" alt="Trazado" src="/img/trazado-2503.png" />
                    <img className="trazado-5" alt="Trazado" src="/img/trazado-2503.png" />
                    <img className="trazado-6" alt="Trazado" src="/img/trazado-2507.png" />
                    <img className="trazado-7" alt="Trazado" src="/img/trazado-2507.png" />
                  </div>
                </div>
              </div>
              <div className="grupo-wrapper">
                <div className="grupo-2">
                  <img className="oval" alt="Oval" src="/img/oval-2.png" />
                  <div className="bell-ring">
                    <div className="overlap-group-3">
                      <img className="trazado-8" alt="Trazado" src="/img/trazado-2510.png" />
                      <img className="trazado-9" alt="Trazado" src="/img/trazado-2512.png" />
                      <img className="trazado-10" alt="Trazado" src="/img/trazado-2513.png" />
                    </div>
                    <img className="trazado-11" alt="Trazado" src="/img/trazado-2511.png" />
                  </div>
                </div>
              </div>
              <p className="empresa-fiber">
                <span className="span">Empresa: </span>
                <span className="text-wrapper-9">Fiber digital</span>
              </p>
              <img className="chevron-right-2" alt="Chevron right" src="/img/chevron-right-3.png" />
              <div className="componente-2">
                <div className="rectngulo-wrapper">
                  <img className="rectngulo-2" alt="Rectngulo" src="/img/rect-ngulo-17.png" />
                </div>
              </div>
            </div>
          </div>
          <div className="text-wrapper-10">Estado de cuenta</div>
          <div className="text-wrapper-11">/ Mes</div>
          <p className="p">Valor presentado en base (Sol)</p>
          <div className="text-wrapper-12">Cliente #TW1500001</div>
          <div className="text-wrapper-13">Balance</div>
          <div className="text-wrapper-14">S/. 3127,00</div>
        </div>
        <div className="overlap-2">
          <img className="rectangle-2" alt="Rectangle" src="/img/rectangle-2-1.png" />
          <div className="rectangle-3" />
          <img className="logo-placeholder" alt="Logo placeholder" src="/img/logo-placeholder-3-removebg-preview.png" />
          <div className="componente-3">
            <div className="overlap-3">
              <div className="text-wrapper-15">Dashboard</div>
              <div className="layout-dashboard">
                <div className="rectngulo-3" />
                <div className="rectngulo-4" />
                <div className="rectngulo-5" />
                <div className="rectngulo-6" />
              </div>
            </div>
            <div className="text-wrapper-16">Cuenta</div>
            <div className="text-wrapper-17">Usuarios</div>
            <div className="text-wrapper-18">Menu</div>
            <img className="chevron-right-3" alt="Chevron right" src="/img/chevron-right-2.png" />
            <img className="chevron-right-4" alt="Chevron right" src="/img/chevron-right-2.png" />
            <div className="wallet">
              <div className="overlap-group-4">
                <img className="trazado-12" alt="Trazado" src="/img/trazado-2497.png" />
                <img className="trazado-13" alt="Trazado" src="/img/trazado-2498.png" />
              </div>
            </div>
            <div className="overlap-4">
              <div className="square-user-round">
                <div className="overlap-5">
                  <img className="trazado-14" alt="Trazado" src="/img/trazado-2499.png" />
                  <div className="elipse-2" />
                  <div className="rectngulo-7" />
                </div>
              </div>
              <img className="oval-2" alt="Oval" src="/img/oval-1.png" />
            </div>
            <img className="oval-3" alt="Oval" src="/img/oval-1.png" />
          </div>
        </div>
        <div className="overlap-6">
          <img className="rectangle-4" alt="Rectangle" src="/img/rectangle.png" />
          <div className="text-wrapper-19">Últimos movimientos</div>
          <div className="grupo-4">
            <p className="text-wrapper-20">13 Mar, 2024 to 21 Mar, 2024</p>
          </div>
          <p className="text-wrapper-21">Visualiza el detalle de tus facturas y pagos</p>
          <div className="componente-4">
            <div className="grupo-5">
              <div className="grupo-6">
                <div className="overlap-7">
                  <div className="text-wrapper-22">F 001-00001753</div>
                  <div className="text-wrapper-23">3127,00</div>
                  <div className="text-wrapper-24">- 3127,00</div>
                  <div className="text-wrapper-25">2013-08-11</div>
                  <div className="text-wrapper-26">Factura F 001-00001753</div>
                </div>
                <div className="overlap-group-5">
                  <div className="text-wrapper-22">F 001-00001753</div>
                  <div className="text-wrapper-27">3127,00</div>
                  <div className="text-wrapper-28">3127,00</div>
                  <div className="text-wrapper-29">2013-08-11</div>
                  <div className="text-wrapper-30">Pago PBNSOL/2024/00105</div>
                </div>
                <div className="overlap-8">
                  <div className="text-wrapper-31">F 001-00001753</div>
                  <div className="text-wrapper-32">3127,00</div>
                  <div className="text-wrapper-33">- 3127,00</div>
                  <div className="text-wrapper-25">2013-08-11</div>
                  <div className="text-wrapper-26">Factura F 001-00001753</div>
                </div>
                <div className="overlap-9">
                  <div className="text-wrapper-31">F 001-00001753</div>
                  <div className="text-wrapper-34">3127,00</div>
                  <div className="text-wrapper-35">3127,00</div>
                  <div className="text-wrapper-25">2013-08-11</div>
                  <div className="text-wrapper-26">Pago PBNSOL/2024/00105</div>
                </div>
                <div className="overlap-10">
                  <div className="text-wrapper-36">F 001-00001753</div>
                  <div className="text-wrapper-37">3127,00</div>
                  <div className="text-wrapper-38">3127,00</div>
                  <div className="text-wrapper-39">2013-08-11</div>
                  <div className="text-wrapper-40">Pago PBNSOL/2024/00105</div>
                </div>
                <div className="overlap-11">
                  <div className="text-wrapper-31">F 001-00001753</div>
                  <div className="text-wrapper-41">3127,00</div>
                  <div className="text-wrapper-42">- 3127,00</div>
                  <div className="text-wrapper-25">2013-08-11</div>
                  <div className="text-wrapper-26">Factura F 001-00001753</div>
                </div>
                <div className="overlap-12">
                  <div className="text-wrapper-31">F 001-00001753</div>
                  <div className="text-wrapper-43">3127,00</div>
                  <div className="text-wrapper-44">3127,00</div>
                  <div className="text-wrapper-45">2013-08-11</div>
                  <div className="text-wrapper-46">Pago PBNSOL/2024/00105</div>
                </div>
                <div className="overlap-13">
                  <div className="text-wrapper-31">F 001-00001753</div>
                  <div className="text-wrapper-47">3127,00</div>
                  <div className="text-wrapper-48">- 3127,00</div>
                  <div className="text-wrapper-45">2013-08-11</div>
                  <div className="text-wrapper-46">Factura F 001-00001753</div>
                </div>
                <div className="overlap-14">
                  <div className="text-wrapper-31">F 001-00001753</div>
                  <div className="text-wrapper-41">3127,00</div>
                  <div className="text-wrapper-42">- 3127,00</div>
                  <div className="text-wrapper-49">2013-08-11</div>
                  <div className="text-wrapper-50">Factura F 001-00001753</div>
                </div>
                <div className="overlap-15">
                  <div className="text-wrapper-31">F 001-00001753</div>
                  <div className="text-wrapper-51">3127,00</div>
                  <div className="text-wrapper-52">3127,00</div>
                  <div className="text-wrapper-25">2013-08-11</div>
                  <div className="text-wrapper-26">Pago PBNSOL/2024/00105</div>
                </div>
                <div className="overlap-16">
                  <div className="text-wrapper-53">Fecha</div>
                  <div className="text-wrapper-54">Descripción</div>
                  <div className="text-wrapper-55">Cargo</div>
                  <div className="text-wrapper-56">Abono</div>
                  <div className="saldo">Saldo&nbsp;&nbsp;&nbsp;&nbsp; </div>
                  <div className="text-wrapper-57">Numero</div>
                </div>
              </div>
              <div className="grupo-7">
                <div className="text-wrapper-58">Sin asociar</div>
                <img className="oval-4" alt="Oval" src="/img/oval-22.png" />
              </div>
            </div>
            <div className="grupo-8">
              <div className="grupo-9">
                <div className="text-wrapper-59">General</div>
              </div>
              <div className="grupo-10">
                <div className="text-wrapper-60">Facturas</div>
              </div>
              <div className="grupo-11">
                <div className="text-wrapper-61">Pagos</div>
              </div>
            </div>
          </div>
          <p className="text-wrapper-62">Mostrando 10 de 200 resultados</p>
          <div className="next">
            <div className="element-wrapper">
              <div className="element" />
            </div>
            <div className="overlap-17">
              <div className="element-2" />
            </div>
            <div className="overlap-18">
              <div className="prev" />
              <div className="frame">
                <div className="overlap-19">
                  <img className="frame-2" alt="Frame" src="/img/frame-copy.png" />
                  <div className="group">
                    <div className="overlap-group-6">
                      <img className="line" alt="Line" src="/img/line-3.png" />
                      <img className="line-2" alt="Line" src="/img/line-1.png" />
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="overlap-20">
              <div className="element-3" />
            </div>
            <div className="overlap-21">
              <div className="next-2" />
              <div className="frame-copy">
                <div className="overlap-group-wrapper">
                  <div className="overlap-group-6">
                    <img className="line-3" alt="Line" src="/img/line-2.png" />
                    <img className="line-4" alt="Line" src="/img/line-3.png" />
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="componente-5">
            <div className="filter-wrapper">
              <img className="filter" alt="Filter" src="/img/filter.png" />
            </div>
          </div>
          <div className="componente-6">
            <div className="grupo-12">
              <div className="download">
                <div className="overlap-group-7">
                  <img className="trazado-15" alt="Trazado" src="/img/trazado-2517.png" />
                  <img className="trazado-16" alt="Trazado" src="/img/trazado-2518.png" />
                  <img className="lnea" alt="Lnea" src="/img/l-nea-56.png" />
                </div>
              </div>
              <div className="text-wrapper-63">Descargar información</div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
