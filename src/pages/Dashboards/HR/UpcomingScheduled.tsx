...
const UpcomingScheduled = () => {
  useEffect(() => {
    document
      .getElementById("upcomingscheduledflatpickr")
            ...
            <div className="flex flex-col gap-4 mt-3">
              {(UpcomingScheduledData || []).map((item: any, key: number) => (
                <div className="flex gap-3" key={key}>
                  <div className="flex flex-col items-center justify-center size-12 border rounded-sm border-slate-200 dark:border-zink-500 shrink-0">
                    <h6>{item.date}</h6>{" "}
                  ...
                  <div className="grow">
                    <h6 className="mb-1">
                      {item.title}{" "}
                      {item.time && (
                        <small className="inline-block px-2 font-medium border border-transparent rounded text-[11px] py-0.5 bg-slate-100 text-slate-500 dark:bg-slate-500/20 dark:text-zink-200 dark:border-transparent">
                          {item.time}
                        </small>
                      )}
                    </h6>
            ...
            <div className="flex gap-3 p-2 mt-3 rounded-md bg-custom-500">
              ...
              <div>
                ...
                <p className="text-custom-200">
                  If you would like to learn more about transferring the license
                  to a customer
                </p>
              </div>
            </div>
