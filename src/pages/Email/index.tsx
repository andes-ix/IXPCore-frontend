...
const Mailbox = () => {
  ...
  const readAll = (ele: any) => {
    if (ele === "unread") {
      document
        .querySelectorAll("tr.relative:not(.unread)")
      ...
      document
        .querySelectorAll("tr.relative.unread")
        .forEach(function (element: any) {
          const ele = element.querySelector("input").value;
          dispatch(onUnreadMail(ele));
        });
          ...
          <div className="card-body">
            ...
            <ul className="mt-4">
              <li>
                <Link
                  to="#!"
                  className={`transition-all duration-300 ease-linear text-slate-500 dark:text-zink-200 hover:text-custom-500 dark:hover:text-custom-500 inline-block py-2 [&.active]:text-custom-500 dark:[&.active]:text-custom-500 ${
                    activeTabs === "all" && "active"
                  }`}
                      ...
                      <tbody className="elmLoader" id="mail-list">
                        ...
                        {filteredMails.length === 0 ? (
                          ...
                          (filteredMails || []).map(
                            (item: any, key: number) => (
                              <tr
                                className={`relative before:absolute ltr:before:left-0 rtl:before:right-0 before:border [&.checked]:before:border-custom-500 before:inset-y-0 before:border-transparent group/mail ${
                                  item.unread ? "unread" : ""
                                }`}
                                key={key}
                              >
