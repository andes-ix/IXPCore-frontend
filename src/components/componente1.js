...
const Componente1: FunctionComponent<Componente1Type> = memo(
  ({ className = "", propTop }) => {
    const componente21Style: CSSProperties = useMemo(() => {
      return {
        top: propTop,
      };
    }, [propTop]);
    ...
    return (
      <div
        className={`absolute top-[calc(50%_-_254.56px)] right-[152.16px] w-[189.5px] h-[167px] text-left text-sm text-lightslategray-100 font-open-sans ${className}`}
        style={componente21Style}
      >
