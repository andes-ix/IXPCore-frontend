...
export type Grupo4Type = {
  className?: string;
};

const Grupo4: FunctionComponent<Grupo4Type> = memo(({ className = "" }) => {
  ...
  return (
    <div
      className={`absolute top-[236px] right-[258px] rounded-3xs bg-white w-[514px] h-[586px] cursor-pointer text-left text-sm text-dimgray-200 font-open-sans ${className}`}
    ...
    >
      ...
      <input
        className="m-0 absolute top-[calc(50%_+_85px)] left-[calc(50%_-_207px)] [filter:drop-shadow(-1.8461538553237915px_4.307692527770996px_8px_#f1f7ff)] rounded-8xs box-border w-5 h-5 border-[1px] border-solid border-gray-400"
        type="checkbox"
      />
      <img
